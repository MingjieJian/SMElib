...
int  NumberSpectralSegments, NLINES, NWAVE_C=0, *INDX_C=NULL;
double WFIRST, WLAST;
...
float **FRACT=NULL, **PARTITION_FUNCTIONS=NULL,
       *POTION=NULL, *MOLWEIGHT=NULL;
short *MARK=NULL, *AUTOION=NULL, *IDHEL=NULL;
int   *ION=NULL, *ANSTEE=NULL;
double *WLCENT=NULL, *EXCIT=NULL, *GF=NULL,
       *GAMRAD=NULL, *GAMQST=NULL, *GAMVW=NULL, *ALMAX=NULL,
...
short flagMODEL, flagWLRANGE, flagABUND, flagLINELIST,
      flagIONIZ, flagCONTIN, lineOPACITIES=0, flagH2broad=0,
      initNLTE=0, *flagNLTE;
...
int    Contrib_SPH(double rhox[][2*MOSIZE], int, int NRHOXs[], double, double,
                   double *, double *, int, int &, double *, short, int grazing[]);
void   Contrib_Intens_PP(int, double *, double *, double *, double *);
void   Contrib_Intens_SPH(int, double *, double *, double *, double *, int);
...
{
   int i, j, l=strlen(s);
   for(i=0; i<l; i++) if(!isspace(s[i])) break;
   for(j=l-1; j>=i; j--) if(isspace(s[j])) s[j]='\0';
   return s+i;
}
...
*/
  int s=0, t=0;
  do if(!isspace(source[s])) target[t++]=source[s]; while(source[s++] != '\0');
  return t-1;
}
...
extern "C" char const * SME_DLL InputWaveRange(int n, void *arg[]) /* Read in Wavelength range */
{
  ...
  if(n<2) {strncpy(result, "Only one argument found", 511); return result;}
  if(flagWLRANGE)
  {
    if(fabs(WFIRST- *(double *)arg[0])<1.e-3 &&
       fabs(WLAST - *(double *)arg[1])<1.e-3) return &OK_response;
  }
  WFIRST=*(double *)arg[0];
  WLAST =*(double *)arg[1];
  ...
  {
    flagWLRANGE=0;
    strncpy(result, "Wrong wavelength range", 511);
    return result;
  }
  else
  {
...
*/
  ...
  if(flagLINELIST)
  {
    ...
    if(AUTOION!=NULL)    FREE(AUTOION);
    if(WLCENT !=NULL)    FREE(WLCENT);
    if(EXCIT  !=NULL)    FREE(EXCIT);
    if(GF     !=NULL)    FREE(GF);
    if(GAMRAD !=NULL)    FREE(GAMRAD);
    if(GAMQST !=NULL)    FREE(GAMQST);
    if(GAMVW  !=NULL)    FREE(GAMVW);
    if(ANSTEE !=NULL)    FREE(ANSTEE);
    if(IDHEL  !=NULL)    FREE(IDHEL);
    if(ALMAX  !=NULL)    FREE(ALMAX);
    if(Wlim_left !=NULL) FREE(Wlim_left);
    if(Wlim_right!=NULL) FREE(Wlim_right);
    flagLINELIST=0;
  }
  ...
  if(lineOPACITIES)
  {
    for(i=0;i<NRHOX;i++)
    {
      if(LINEOP[i] !=NULL) FREE(LINEOP[i]);
      if(AVOIGT[i] !=NULL) FREE(AVOIGT[i]);
      if(VVOIGT[i] !=NULL) FREE(VVOIGT[i]);
    }
    lineOPACITIES=0;
  }
  ...
  for(LINE=0; LINE<NLINES-1; LINE++)
  {
    ...
    {
      flagLINELIST=0;
      strncpy(result, "Line list is not sorted in wavelength ascending order", 511);
      return result;
    }
  }
...
/* spname will be passed to FORTRAN, so no trailing zero's, fixed length
   padded with spaces instead */
    ...
    if(a0[LINE].slen<8) for(l=a0[LINE].slen; l<8; l++) spname[8*LINE+l]=' ';
//    ION[LINE]   =(int)a2[LINE]; /* Ionization            */
    for(l=0; l<a0[LINE].slen; l++) if(*(a0[LINE].s+l)==' ') break;
    ...
    if(EXCIT[LINE]>100.) EXCIT[LINE]=EXCIT[LINE]/8065.544;
    if(GAMRAD[LINE]<20. && GAMRAD[LINE]>0.) GAMRAD[LINE]=pow10(GAMRAD[LINE]);
    GRLG10=0.;
    if(GAMRAD[LINE]>0.) GRLG10=log10(GAMRAD[LINE]);
    if(strncmp(spname+8*LINE, "H 1", 3))  /* Non-Hydrogen line */
    {
      if(GAMQST[LINE]<0.) GAMQST[LINE]=pow10(GAMQST[LINE]);
      GSLG10=0.;
      if(GAMQST[LINE]>0.) GSLG10=log10(GAMQST[LINE]);
      if(GAMVW[LINE] <0.)
      {
        GAMVW[LINE]=pow10(GAMVW[LINE]);
        GWLG10=0.;
        if(GAMVW[LINE] >0.) GWLG10=log10(GAMVW[LINE]);
        ANSTEE[LINE]=0;
      }
...
*/
  ...
  if(n<2) {strncpy(result, "Not enough arguments", 511); return result;}
  if(!flagLINELIST) {strncpy(result, "No line list", 511); return result;}
  Nlines=*(int *)arg[0];
  if(NLINES<1)
  {
    flagLINELIST=0;
  ...
  for(LINE=0; LINE<min(Nlines, NLINES); LINE++)
  {
    ...
    if(strncmp(spname+8*LINE, "H ", 2))  /* Non-Hydrogen line     */
    {
      ...
      a1[6*LINE+5]=(GAMVW[LINE] >0.&&
                    GAMVW[LINE]<5.)?log10(GAMVW[LINE]):GAMVW[LINE]; /* Van der Waals damping */
    }
    else                                 /* Hydrogen line         */
    {
...
*/
  ...
  for(LINE=0; LINE<NUPDTE; LINE++)
  {
    ...
    if(i<0 || i>=NLINES)
    {
      strncpy(result, "Replacement index is out of range", 511);
      return result;
    }
...
/* spname will be passed to FORTRAN, so no trailing
   zero's, fixed length padded with spaces instead */
    ...
    if(a0[LINE].slen<8) for(l=a0[LINE].slen; l<8; l++) tmpname[l]=' ';
    WW =a3[LINE];                            /* Wavelength */
    EXC=a4[LINE]; if(EXC>100.) EXC/=8065.544; /* Excitation */
...
/* Make sure we are talking about the same line.
   Check species name and excitation potential */
    ...
    {
      snprintf(ERRMES, 59, "Attempt to replace line %d with another line", i);
    ...
    GAMQST[i]=a7[LINE];
    GAMVW[i] =a8[LINE];
    if(GAMRAD[i]<20. && GAMRAD[i]>0.) GAMRAD[i]=pow10(GAMRAD[i]);
    GRLG10=0.;
    if(GAMRAD[i]>0.) GRLG10=log10(GAMRAD[i]);
    if(strncmp(spname+8*i, "H ", 2)) /* Non-Hydrogen line */
    {
      if(GAMQST[i]<0.) GAMQST[i]=pow10(GAMQST[i]);
      GSLG10=0.;
      if(GAMQST[i]>0.) GSLG10=log10(GAMQST[i]);
      if(GAMVW[i] <0.) GAMVW[i]=pow10(GAMVW[i]);
      GWLG10=0.;
      if(GAMVW[i] >0.) GWLG10=log10(GAMVW[i]);
    }
...
// Free invalidated arrays
  ...
  flagMODEL=0; flagCONTIN=0; lineOPACITIES=0;

  NRHOX=*(short *)arg[0];
  if(NRHOX>MOSIZE)
  {
    snprintf(result, 511,
            "SME library supports atmospheric model with maximum %d depth layers", MOSIZE);
    return result;
  }
  ...
  s=(IDL_STRING *)arg[4]; l=min(4,s->slen);
  strncpy(motype, s->s, l); motype[l]=0;
  for(i=0; i<strlen(motype); i++) motype[i]=toupper(motype[i]);

// Adding provision for spherical models
  if(!strncmp(motype, "TAU", 3))
  {
    MOTYPE=0; arg_offset=0;
    RADIUS=-1.;
  }
  else if(!strncmp(motype, "RHOX", 4))
  {
    MOTYPE=1;  arg_offset=0;
    RADIUS=-1.;
  }
  else if(!strncmp(motype, "SPH", 3))
  {
    MOTYPE=3; arg_offset=1;
    RADIUS=*(double *)arg[5];
  }

  ifop=(short *)arg[5+arg_offset];
  for(i=0; i<20; i++) IFOP[i]=ifop[i];
...
// Allocate space for the line opacities and Voigt parameters
  ...
  if(MOTYPE==3) a7=(double *)arg[12+arg_offset];
...
*/
{
  ...
  {
    strncpy(result, "No arguments found", 511);
    return result;
  }
  if(!flagMODEL)
  {
    strncpy(result, "Model atmosphere must be set before departure coefficients", 511);
    return result;
  }
  if(!flagLINELIST)
  {
    strncpy(result, "Line list must be set before departure coefficients", 511);
    return result;
  }

  if(!initNLTE) // Initialize the departure arrays for the first time
  {
    for(im=0; im<MOSIZE; im++) LTE_b[im]=1.; // Initialize the default LTE b's
  ...
  {
    strncpy(result, "Attempt to set departure coefficients for non-existing transition", 511);
    return result;
  }
                                                                                ...
                                                                                specific line */
{
  ...
  {
    strncpy(result, "Requires an array pointer, its length and line number", 511);
    return result;
  }

  if(!initNLTE)
  {
    strncpy(result, "NLTE mode was not initialized. No departure coefficients available.", 511);
    return result;
  }
...
extern "C" char const * SME_DLL GetNLTEflags(int n, void *arg[]) /* Get NLTE flag for every line */
{
  ...
  {
    strncpy(result, "GetNLTELines: Requires an array pointer and its length", 511);
    return result;
  }
...
*/
  ...
  {
    if((MOTYPE!=0 && n<3) ||
       (MOTYPE==0 && n<4)) {strncpy(result, "Opacity: Not enough arguments", 511); return result;}
  }
  if(!flagMODEL) {strncpy(result, "Model atmosphere not set", 511); return result;}
  if(!flagWLRANGE)
  {
    strncpy(result, "Wavelength interval was not specified", 511); return result;
  }
  if(!flagABUND) {strncpy(result, "Abundances were not set", 511); return result;}
...
// Continuous opacity at the blue edge
  ...
  if(n>=3)
  {
    ...
    a1=(double *)arg[1];
    a2=(double *)arg[2];
    if(MOTYPE==0) a3=(double *)arg[3];
    for(i=0; i<nrhox; i++)
    {
      a1[i]=COPBLU[i];
      a2[i]=COPRED[i];
      if(n>=4 && MOTYPE==0) a3[i]=COPSTD[i];
    }
  }
...
/*  CLEAR OPACITY ACCUMULATORS */
  ...
  for(J=0; J<NRHOX; J++)
  {
    AHYD[J]=0; AH2P[J]=0; AHMIN[J]=0;
    SIGH[J]=0; AHE1[J]=0; AHE2[J]=0;
    AHEMIN[J]=0; SIGHE[J]=0; ACOOL[J]=0;
    ALUKE[J]=0; AHOT[J]=0; SIGEL[J]=0; SIGH2[J]=0;
  }

  if(IFOP[ 0]==1) HOP(AHYD, IXH1, IXH2);
  if(IFOP[ 1]==1) H2PLOP(AH2P, IXH1, IXH2);
  if(IFOP[ 2]==1) HMINOP(AHMIN, IXH1, IXHMIN);
  if(IFOP[ 3]==1) HRAYOP(SIGH, IXH1);
  if(IFOP[ 4]==1) HE1OP_new(AHE1, IXHE1, IXHE2);
...
double COULBF1S(double FREQ, double Z)
{
  ...
  coulbf1s=0.;
  if(FREQ/(Z*Z)<3.28805e15) return 0.;
         ...
         double XNEW[], double FNEW[], int NNEW)
{
  ...
  for(K=0; K<NNEW; K++)
  {
    ...
    if(L>2)
    {
      ...
      WT=0.;
      if(fabs(CFOR)!=0.) WT=fabs(CFOR)/(fabs(CFOR)+fabs(CBAC));
      A=AFOR+WT*(ABAC-AFOR);
      B=BFOR+WT*(BBAC-BFOR);
      C=CFOR+WT*(CBAC-CFOR);
      LL=L;
...
//     Karzas, W.J. and Latter, R. 1961, ApJS 6, 167-212.
  ...
  if(N<=15)
  {
    if(L>=N || N>6)
    {
      ...
      for(I=2; I<30; I++)
      {
        if(FREQLG>FREQN[N-1][I-1]) break;
      }
      X=(FREQLG-FREQN[N-1][I-1])/(FREQN[N-1][I-2]-FREQN[N-1][I-1])*
                (XN[N-1][I-2]-XN[N-1][I-1])+XN[N-1][I-1];
...
double COULX(int N, double freq, double Z)
{
  ...
  n=(N+1)*(N+1);
  if(freq<Z*Z*3.28805e15/n) return 0.;
...
void HOP(double *ahyd, int iH1, int iH2) /* REQUIRES FUNCTIONS COULX AND COULFF */
{
  ...
  {
    nH1=FRACT[J][iH1];
    for(N=0; N<8; N++) BOLT[J][N]=exp(-13.595*(1.-1./(N+1)/(N+1))/TKEV[J])*
                                  2.*(N+1)*(N+1)*nH1/RHO[J];
...
//  XR=FRACT[J][iH1]/13.595*TKEV[J]/RHO[J];
  ...
  for(N=0; N<8; N++) CONT[N]=COULX(N, FREQ, 1.);
/*
    FREQ3=FREQ**3
...
*/
  ...
  {
    EX=BOLTEX[J];
    if(FREQ<4.05933e13) EX=EXLIM[J]/EHVKT[J];
    H=(CONT[6]*BOLT[J][6]+CONT[7]*BOLT[J][7]+(EX-EXLIM[J])*C+
       COULFF(J, 1)*FREET[J]/FREQ*CFREE)*STIM[J];
    for(N=0; N<6; N++) H+=CONT[N]*BOLT[J][N]*(1.-EHVKT[J]);
    ahyd[J]=H;
  }
  return;
}
...
void HRAYOP(double *sigh, int iH1)
{
  ...
  for(J=0; J<NRHOX; J++) sigh[J]=SIG*FRACT[J][iH1]*2./RHO[J];
  return;
}
...
void HMINOP_old(double *ahmin, int iH1, int iHmin)
{
  ...
  else if(FREQ>=2.111e14) HMINBF=6.801e-10+(5.358e-3+(1.481e3+(-5.519e7+
                                 4.808e11/FREQ1)/FREQ1)/FREQ1)/FREQ1;
...
//    HMINFF=(B+C/T[J])*FRACT[J][iH1]*XNE[J]*2.e-20;
    ...
    if(T[J]>7730.) HMINFR=exp(0.7552/TKEV[J])/(2.*2.4148E15*T[J]*
                          sqrt(T[J]))*FRACT[J][iH1]*XNE[J];
// Bug fixed 2007-12-15: Partition function of H- is 1 and not 2 as we used
// before:
    else           HMINFR=FRACT[J][iHmin];
...
// 91.134 number taken from Bell and Berrington
    ...
    for(ITHETA=0; ITHETA<11; ITHETA++) FFLOG[ITHETA][IWAVE]=log(FF[IWAVE][ITHETA]*1.e-26);
  }
...
// .754209 Hotop and Lineberger J. Phys. Chem. Ref. Data Vol. 14, 731-752, 1985
  ...
  HMINBF[0]=0.;
  if(FREQ > 1.82365E14) MAXWAVE=MAP1(WBF, BF, 85, WAVE, HMINBF, 1);
  for(J=0; J<NRHOX; J++)
  {
...
/*    XRLOG=log(FRACT[J][iHe1]*(4/2/13.595)*TKEV[J]/RHO[J]); */
  ...
  FREQ3=FREQ*1.e-10; FREQ3=FREQ3*FREQ3*FREQ3;
  CFREE=3.6919e8/FREQ3;
  C=2.815e-1/FREQ3;
  ...
  switch(IMIN)
  {
    case 0: TRANS[0]=exp(33.32e0-2.e0*FREQLG);
    case 1: TRANS[1]=exp(-390.026e0+(21.035e0-0.318e0*FREQLG)*FREQLG);
    case 2: TRANS[2]=exp(26.83e0-1.91e0*FREQLG);
    case 3: TRANS[3]=exp(61.21e0-2.9e0*FREQLG);
    case 4: TRANS[4]=exp(81.35e0-3.5e0*FREQLG);
    case 5: TRANS[5]=exp(12.69e0-1.54e0*FREQLG);
    case 6: TRANS[6]=exp(23.85e0-1.86e0*FREQLG);
    case 7: TRANS[7]=exp(49.30e0-2.60e0*FREQLG);
    case 8: TRANS[8]=exp(85.20e0-3.69e0*FREQLG);
    case 9: TRANS[9]=exp(58.81e0-2.89e0*FREQLG);
    default: break;
  }
  ...
  {
    EX=BOLTEX[J];
    if(FREQ<2.055e14) EX=EXLIM[J]/EHVKT[J];
    HE1=(EX-EXLIM[J])*C;
    for(N=0; N<10; N++) HE1+=TRANS[N]*BOLT[J][N];
...
// Following Mathisen
  ...
  if(FREQ < 5.945209e15) return 0.;
  WAVE=CLIGHT/FREQ;
  for(i=1; i<64; i++) if(WAVE > W[i]) break;
...
double HE12s1S(double FREQ)
{
  ...
  FREQLG10=log10(FREQ);
  for(i=1; i<16; i++) if(FREQLG10 > FREQ1S[i]) break;
...
double HE12s3S(double FREQ)
{
  ...
  FREQLG10=log10(FREQ);
  for(i=1; i<16; i++) if(FREQLG10 > FREQ3S[i]) break;
...
double HE12p1P( double FREQ)
{
  ...
  FREQLG10=log10(FREQ);
  for(i=1; i<16; i++) if(FREQLG10 > FREQ1P[i]) break;
...
double HE12p3P(double FREQ)
{
  ...
  if(FREQ < 29223.753*CLIGHTcm) return 0.;
  FREQLG10=log10(FREQ);
  for(i=1; i<16; i++) if(FREQLG10 > FREQ3P[i]) break;
...
//    ahe1[J]=0.1;
  ...
  switch(IMIN)
  {
    ...
    case   1: TRANS[0]=CROSSHE(FREQ);
    case   2: TRANS[1]=HE12s3S(FREQ);
    case   3: TRANS[2]=HE12s1S(FREQ);
    case   4: TRANS[3]=HE12p3P(FREQ);
    case   5: TRANS[4]=HE12p1P(FREQ);
    case   6: TRANS[5]=XKARZAS(FREQ, 1.236439e0, 3, 0); // 1s3s 3S
    case   7: TRANS[6]=XKARZAS(FREQ, 1.102898e0, 3, 0); // 1s3s 1S
    case   8: TRANS[7]=XKARZAS(FREQ, 1.045499e0, 3, 1); // 1s3p 3P
    case   9: TRANS[8]=XKARZAS(FREQ, 1.001427e0, 3, 2); // 1s3d 3D+1D
    case  10: TRANS[9]=XKARZAS(FREQ, 0.9926e0,   3, 1); // 1s3p 1P
    default: break;
  }
// HeII n=2
  ELIM=527490.06e0;
...
//  return;
  for(J=0; J<NRHOX; J++)
  {
    EX=(FREQ < 2.055e14)?EXLIM[J]/EHVKT[J]:BOLTEX[J];
    HE1=(EX-EXLIM[J])*C;
    for(N=IMIN-1; N<10; N++) HE1+=TRANS[N]*BOLT[N][J];
    if(FREQ >= 1.25408e16)
    {
      for(N=3; N<27; N++) HE1+=TRANSN[N]*BOLTN[N][J];
    }
    ahe1[J]=(HE1+COULFF(J,1)*FREET[J]*CFREE)*STIM[J];
  }
}
...
/* FREQUENCIES ARE 4X HYDROGEN, CHI ARE FOR ION POT=54.403 */
  ...
  for(J=0; J<NRHOX; J++)
  {
    for(N=0; N<9; N++)
    {
      ...
      BOLT[J][N]=(FRACT[J][iHe2]==0.0 || BLTARG>80.)?0.:
                  exp(-BLTARG)*2.*(N+1)*(N+1)*FRACT[J][iHe2];
    }
...
/*    XRLOG=log(TKEV[J]*(2/2/13.595)/RHO[J]); */
    ...
    BOLTEX[J]=(FRACT[J][iHe2]==0.0 || BLTLOG>80.)?0.:
               FRACT[J][iHe2]*exp(-BLTLOG);
    EXLLOG=54.403/TKEV[J]-XRLOG;
    EXLIM[J]=(FRACT[J][iHe2]==0.0 || EXLLOG>80.)?0.:
              FRACT[J][iHe2]*exp(-EXLLOG);
  }
//  for(N=0; N<9; N++) CONT[N]=COULX(N, FREQ, 2.);
  for(N=0; N<9; N++) CONT[N]=XKARZAS(FREQ, 4.e0, N+1, N+1);
  FREQ3=(FREQ*1.e-05); FREQ3=FREQ3*FREQ3*FREQ3;
  CFREE=3.6919e-07/FREQ3*4.;
  C=2.815e14*2.*2./FREQ3;
  for(J=0; J<NRHOX; J++)
  {
    EX=BOLTEX[J];
    if(FREQ<1.31522e14) EX=EXLIM[J]/EHVKT[J];
    HE2=(EX-EXLIM[J])*C;
    for(N=0; N<9; N++) HE2=HE2+CONT[N]*BOLT[J][N];
...
void HERAOP(double *sighe, int iHe1)
{
  ...
  for(J=0; J<NRHOX; J++) sighe[J]=SIG*FRACT[J][iHe1]/RHO[J];
  return;
}
...
double C1OP(int J) /* CROSS-SECTION */
{
  ...
  X1444=0.; X1240=0.; X1100=0.;
  if(FREQ>=2.7254e15) X1100=SEATON(2.7254e15,1.219e-17,2.0,3.317);
  if(FREQ>=2.4196e15) X1240=SEATON(2.4196e15,1.030e-17,1.5,2.789);
  if(FREQ>=2.0761e15) X1444=SEATON(2.0761e15,9.590e-18,1.5,3.501);
...
{                      /* This routine is based on R.L. Kurucz Atlas12 */  
  static double ELEV[25]={79314.86, 78731.27, 78529.62, 78309.76, 78226.35,
                ...
                GLEV[25]={9.,3.,7.,15.,21.,5.,1.,5.,9.,3.,15.,3.,3.,9.,1.,5.,5.,
                          3.,1.,3.,3.,5.,12.,15.,5.}, RYD=109732.298;
...
//  2s2 2p3d 3P
//  ELEV=79314.86
    if(WAVENO<ELIM-ELEV[0]) break;
//  GLEV=9.
    ZEFF2=9./RYD*(ELIM-ELEV[0]);
    X[0]=XKARZAS(FREQ, ZEFF2, 3, 2);
//  2s2 2p3d 1P
//  ELEV=78731.27
    if(WAVENO<ELIM-ELEV[1]) break;
...
//  N=4 TO INFINITY
  H=FREQ3*GFACTOR*2./2./(RYD*Z*Z*HCKT)*
    ( exp(-max(ELIM-RYD*Z*Z/16.,ELIM-WAVENO)*HCKT)
     -exp(-ELIM*HCKT));
//  printf("%d %g %g %g %g %g\n", J, H, ELIM, WAVENO, ELIM-WAVENO, HCKT);
//  C II 2s 2p2 4P1/2
//  ELIM=90820.42+43003.3
  for(i=0; i<25; i++) H+=X[i]*BOLT[i];
  return H;
}
...
// TEMP: 4000     5000     6000     7000     8000     9000    10000     WAVE(A)
  ...
  NT=min(6,(int)floor(T[J]/1000.)-3); if(NT<1) NT=1;
...
//     ELEV=54676.710
  ...
  {
    H=FREQ3*GFACTOR*2./2./(RYD*Z*Z*HCKT)*
      (exp(-max(ELIM-RYD*Z*Z/25., ELIM-WAVENO)*HCKT)
      -exp(-ELIM*HCKT));
...
/*     3P,1D,1S,1D,3D,3F,1D,3P */
  ...
  NT=min(8,(int)floor(T[J]/1000.)-3); if(NT<1) NT=1;
  DT=(TLOG[J]-TLG[NT-1])/(TLG[NT]-TLG[NT-1]);
...
// GLEV=5.
// fits to Nahar, S.N. and Pradhan, A.K. J.Phys.B 26, 1109-1127, 1993.
    if(WAVENO<=74000.e0) X[24]=72.e-18*pow(65524.393e0/WAVENO,1.90)/3.;
    else                 X[24]=93.e-18*pow(65524.393e0/WAVENO,4.00)/3.;
// 3s2 3p2 3P1
// ELEV=77.115
    if(WAVENO<ELIM-ELEV[25]) break;

// GLEV=3.
// fits to Nahar, S.N. and Pradhan, A.K. J.Phys.B 26, 1109-1127, 1993.
    if(WAVENO<=74000.e0) X[25]=72.e-18*pow(65524.393e0/WAVENO,1.90)*2./3.;
    else                 X[25]=93.e-18*pow(65524.393e0/WAVENO,4.00)*2./3.;
// 3s2 3p2 3P0
// ELEV=0.00
    if(WAVENO<ELIM-ELEV[26]) break;

// GLEV=1.
// fits to Nahar, S.N. and Pradhan, A.K. J.Phys.B 26, 1109-1127, 1993.
    if(WAVENO<=74000.e0) X[26]=72.e-18*pow(65524.393e0/WAVENO,1.90)/3.;
    else                 X[26]=93.e-18*pow(65524.393e0/WAVENO,4.00)/3.;
    break;
  }
...
// GLEV=5.
// fits to Nahar, S.N. and Pradhan, A.K. J.Phys.B 26, 1109-1127, 1993.
    if(WAVENO<=74000.e0) X[24]+=72.e-18*pow(65524.393e0/WAVENO,1.90)*2./3.;
    else                 X[24]+=93.e-18*pow(65524.393e0/WAVENO,4.00)*2./3.;
// 3s2 3p2 3P1
// ELEV=77.115
    if(WAVENO<ELIM-ELEV[25]) break;
...
// GLEV=1.
// fits to Nahar, S.N. and Pradhan, A.K. J.Phys.B 26, 1109-1127, 1993.
    if(WAVENO<=74000.e0) X[26]+=72.e-18*pow(65524.393e0/WAVENO,1.90)*2./3.;
    else                 X[26]+=93.e-18*pow(65524.393e0/WAVENO,4.00)*2./3.;
    break;
  }
...
// N=5 TO INFINITY
  ...
  for(I=0; I<33; I++) aSi1op+=X[I]*BOLT[I];
  return aSi1op; 
}
...
//  static double Ebin[78], GCROSS[12001][78];
  static double Ebin[44], GCROSS[2401][44];
//  static char *filename="Fe1_Bautista2017.dat.INTEL";
  static char filename[29]="Fe1_Zatsarinny2019.dat.INTEL";
  ...
  if(first)
  {
    ...
    strncat(path, filename, 29);
    fe1op_data=fopen(path, "rb");
    if(fe1op_data==NULL)
    {
      printf("Neutral Iron continuous opacity tables not found\n");
      printf("'%s'\n", path);
      return 0.;
    }

    i=fread(&headlen, sizeof(int), 1, fe1op_data);
    if(change_byte_order) headlen=*(int *)ByteSwap((char *)&headlen, 4);
    i=fread(head, 1, headlen, fe1op_data);

    i=fread(&delta, sizeof(float), 1, fe1op_data);
    if(change_byte_order) delta=*(float *)ByteSwap((char *)&delta, 4);

    i=fread(&n_Ebin, sizeof(int), 1, fe1op_data);
    if(change_byte_order) n_Ebin=*(int *)ByteSwap((char *)&n_Ebin, 4);
    i=fread(Ebin, sizeof(double), n_Ebin, fe1op_data);
    if(change_byte_order)
    {
      for(i_en=0; i_en<n_Ebin; i_en++) Ebin[i_en]=*(double *)ByteSwap((char *)(Ebin+i_en), 8);
    }

    i=fread(&n_WN, sizeof(int), 1, fe1op_data);
    if(change_byte_order) n_WN=*(int *)ByteSwap((char *)&n_WN, 4);

    i=fread(&WN0, sizeof(double), 1, fe1op_data);
    if(change_byte_order) WN0=*(double *)ByteSwap((char *)&WN0, 8);

    i=fread(&WNSTEP, sizeof(double), 1, fe1op_data);
    if(change_byte_order) WNSTEP=*(double *)ByteSwap((char *)&WNSTEP, 8);
    ...
    if(change_byte_order)
    {
      for(i_en=0; i_en<n_Ebin; i_en++)
        ...
        {
          GCROSS[i_en][i_wn]=*(double *)ByteSwap((char *)(GCROSS+i_wn*44+i_en), 8);
        }
    }
    fclose(fe1op_data);
    first=0;
  ...
  if(WAVENO<WN0 || WAVENO>WN0+WNSTEP*(n_WN-1)) return 0.;
...
{                  /* This routine is based on data provided by Phillip Stancil */
  ...
  if(first)
  {
    ...
    strncpy(path, PATH, PATHLEN+1);
    strncat(path, "NH_Stancil2018.dat.INTEL", strlen("NH_Stancil2018.dat.INTEL")+1);
    ...
    if(change_byte_order) gauss_fwhm=*(float *)ByteSwap((char *)&gauss_fwhm, 4);

    i=fread(&n_etrans, sizeof(int), 1, NHop_data);
    if(change_byte_order) n_etrans=*(int *)ByteSwap((char *)&n_etrans, 4);

    i=fread(&n_Temp, sizeof(int), 1, NHop_data);
    if(change_byte_order) n_Temp=*(int *)ByteSwap((char *)&n_Temp, 4);

    i=fread(&n_WL, sizeof(int), 1, NHop_data);
    if(change_byte_order) n_WL=*(int *)ByteSwap((char *)&n_WL, 4);

    i=fread(&WL0, sizeof(double), 1, NHop_data);
    if(change_byte_order) WL0=*(double *)ByteSwap((char *)&WL0, 8);

    i=fread(&WLSTEP, sizeof(double), 1, NHop_data);
    if(change_byte_order) WLSTEP=*(double *)ByteSwap((char *)&WLSTEP, 8);

    i=fread(T_TBL, sizeof(float), n_Temp, NHop_data);
    if(change_byte_order)
    {
      for(i_temp=0; i_temp<n_Temp; i_temp++) T_TBL[i_temp]=*(float *)ByteSwap((char *)(T_TBL+i_temp), 4);
    }
  ...
  WAVE=CLIGHT/FREQ;
  if(WAVE<WL0 || WAVE>WL0+WLSTEP*(n_WL-1)) return 0.;
  if(T[J]<T_TBL[0] || T[J]>T_TBL[n_Temp-1]) return 0.;

  i_wl=(WAVE-WL0)/WLSTEP;
  factor_wl=(WAVE-WL0-i_wl*WLSTEP)/WLSTEP;

  for(i_temp=0; i_temp<n_Temp-1; i_temp++) if(T_TBL[i_temp+1]>T[J]) break;
...
/*
      acool[J]=( C1OP(J)*FRACT[J][IXC1 ]
               ...
               *STIM[J]/RHO[J];
*/
      acool[J]=( C1OP_new(J)*FRACT[J][IXC1 ]
               +MG1OP_new(J)*FRACT[J][IXMG1]
               +AL1OP_new(J)*FRACT[J][IXAL1]
               +SI1OP_new(J)*FRACT[J][IXSI1]
               +FE1OP_new(J)*FRACT[J][IXFE1]
               +CHOP(J)*FRACT[J][IXCH]
               +NHOP(J)*FRACT[J][IXNH]
               +OHOP(J)*FRACT[J][IXOH])
               *STIM[J]/RHO[J];
...
double N1OP(int J) /* Cross-section */
{
  ...
  X1130=0.; X1020=0.; X853=0.;
  if(FREQ>=3.517915e15)  X853=SEATON(3.517915e15,1.142e-17,2.0,4.29);
  if(FREQ>=2.941534e15) X1020=SEATON(2.941534e15,4.410e-18,1.5,3.85);
  if(FREQ>=2.653317e15) X1130=SEATON(2.653317e15,4.200e-18,1.5,4.34);
...
double MG2OP(int J) /* CROSS-SECTION TIMES PARTITION FUNCTION */
{
  ...
  if(FREQ>=3.635492E15) X824=SEATON(3.635492E15,1.40E-19,4.,6.7);
  if(FREQ>=2.564306E15)
  {
    XXX=(2.564306E15/FREQ); XXX=XXX*XXX*XXX; X1169=5.11E-19*XXX;
  }
...
/*     2P,2D,2P,2D,2P */
  ...
  NT=min(5,(int)floor(T[J]/2000.)-4); if(NT<1) NT=1;
  DT=(TLOG[J]-TLG[NT-1])/(TLG[NT]-TLG[NT-1]);
...
double CA2OP(int J) /* CROSS-SECTION TIMES THE PARTITION FUNCTION */
{
  ...
  if(FREQ>=2.110779e15) X1420=SEATON(2.110779e15,4.13e-18,3.,0.69);
...
//  double N1OP(int), O1OP(int), MG2OP(int), SI2OP(int), CA2OP(int);
  ...
  for(J=0; J<NRHOX; J++)
    aluke[J]=( N1OP(J)*FRACT[J][IXN1 ]+ O1OP(J)*FRACT[J][IXO1]+
              ...
              CA2OP(J)*FRACT[J][IXCA2])*STIM[J]/RHO[J];
  return;
}
...
void HOTOP(double *ahot)
{
  ...
  {
    TEMP=T[ITAU]; XNELEC=XNE[ITAU]; XNATOM=XNA[ITAU];
    J=2; MAXION=IONSIZ=6;
    I= 6; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFC +6*ITAU, J);   /* C  */
    I= 7; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFN +6*ITAU, J);   /* N  */
    I= 8; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFO +6*ITAU, J);   /* O  */
    I=10; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFNE+6*ITAU, J);   /* Ne */
    I=12; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFMG+6*ITAU, J);   /* Mg */
    I=14; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFSI+6*ITAU, J);   /* Si */
    I=16; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFS +6*ITAU, J);   /* S  */
    MAXION=IONSIZ=5;
    I=26; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFFE+6*ITAU, J);

    J=1; MAXION=IONSIZ=4;
    I= 6; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFP+21*ITAU   , J); /* C  */
    MAXION=IONSIZ=5;
    I= 7; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFP+21*ITAU+ 4, J); /* N  */
    MAXION=IONSIZ=6;
    I= 8; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFP+21*ITAU+ 9, J); /* O  */
    I=10; xsaha_(I, TEMP, XNELEC, XNATOM, MAXION, POTI, XNFP+21*ITAU+15, J); /* Ne */
  }
/* FREE-FREE */
  ...
  {
    int J2, J3, J4, J5, J6;
    J2=J*6+1; J3=J2+1; J4=J3+1; J5=J4+1; J6=J5+1;
    FREE=COULFF(J,1)* 1.*(XNFC [J2]+XNFN [J2]+XNFO[J2]+XNFNE[J2]+
                          XNFMG[J2]+XNFSI[J2]+XNFS[J2]+XNFFE[J2])+
         COULFF(J,2)* 4.*(XNFC [J3]+XNFN [J3]+XNFO[J3]+XNFNE[J3]+
                          XNFMG[J3]+XNFSI[J3]+XNFS[J3]+XNFFE[J3])+
         COULFF(J,3)* 9.*(XNFC [J4]+XNFN [J4]+XNFO[J4]+XNFNE[J4]+
                          XNFMG[J4]+XNFSI[J4]+XNFS[J4]+XNFFE[J4])+
         COULFF(J,4)*16.*(XNFC [J5]+XNFN [J5]+XNFO[J5]+XNFNE[J5]+
                          XNFMG[J5]+XNFSI[J5]+XNFS[J5]+XNFFE[J5])+
         COULFF(J,5)*25.*(XNFC [J6]+XNFN [J6]+XNFO[J6]+XNFNE[J6]+
                          XNFMG[J6]+XNFSI[J6]+XNFS[J6]          );
  ...
  for(I=1; I<=NUM; I++)
  {
    ...
    for(J=0; J<NRHOX; J++)
    {
      ...
      if(XX>ahot[J]/100.) ahot[J]+=XX/exp(A[L+5]/TKEV[J]);
    }
  }
  for(J=0; J<NRHOX; J++)
  {
...
void ELECOP(double *sigel)
{
  ...
  for(J=0; J<NRHOX; J++) sigel[J]=0.6653e-24*XNE[J]/RHO[J];
}
...
//  double N1OP(int), O1OP(int), MG2OP(int), SI2OP(int), CA2OP(int);
  ...
  if(!flagCONTIN)
  {
    strncpy(result, "Opacity has not been calculated", 511);
    return result;
  }
  ...
  switch(j)
  {
    case -3: for(i=0; i<nrhox; i++) a1[i]=COPSTD[i]; return &OK_response;
    case -2: for(i=0; i<nrhox; i++) a1[i]=COPRED[i]; return &OK_response;
    case -1: for(i=0; i<nrhox; i++) a1[i]=COPBLU[i]; return &OK_response;
    case  0: for(i=0; i<nrhox; i++) a1[i]=AHYD[i];   return &OK_response;
    case  1: for(i=0; i<nrhox; i++) a1[i]=AH2P[i];   return &OK_response;
    case  2: for(i=0; i<nrhox; i++) a1[i]=AHMIN[i];  return &OK_response;
    case  3: for(i=0; i<nrhox; i++) a1[i]=SIGH[i];   return &OK_response;
    case  4: for(i=0; i<nrhox; i++) a1[i]=AHE1[i];   return &OK_response;
    case  5: for(i=0; i<nrhox; i++) a1[i]=AHE2[i];   return &OK_response;
    case  6: for(i=0; i<nrhox; i++) a1[i]=AHEMIN[i]; return &OK_response;
    case  7: for(i=0; i<nrhox; i++) a1[i]=SIGHE[i];  return &OK_response;
    case  8: if(n>3)
             {
               ...
               {
                 a4=(IDL_STRING *)arg[4];
                 if(!strncmp(a4->s, "new", a4->slen)) key=1;
                 if(!strncmp(a4->s, "old", a4->slen)) key=2;
                 if(!strncmp(a4->s, "fraction", a4->slen)) key=3;
               }
               if(!strcmp(species->s, "C1"))
               {
                 switch(key)
                 {
                   case 0: for(i=0; i<nrhox; i++) a1[i]=C1OP_new(i)*FRACT[i][IXC1 ]*STIM[i]/RHO[i];
                           return &OK_response;
                   case 1: for(i=0; i<nrhox; i++) a1[i]=C1OP_new(i);
                           return &OK_response;
                   case 2: for(i=0; i<nrhox; i++) a1[i]=C1OP(i);
                           return &OK_response;
                   case 3: for(i=0; i<nrhox; i++) a1[i]=FRACT[i][IXC1 ]*STIM[i]/RHO[i];
                           return &OK_response;
                 }
               }
               else if(!strcmp(species->s, "Mg1"))
               {
                 switch(key)
                 {
                   case 0: for(i=0; i<nrhox; i++) a1[i]=MG1OP_new(i)*FRACT[i][IXMG1]*STIM[i]/RHO[i];
                           return &OK_response;
                   case 1: for(i=0; i<nrhox; i++) a1[i]=MG1OP_new(i);
                           return &OK_response;
                   case 2: for(i=0; i<nrhox; i++) a1[i]=MG1OP(i);
                           return &OK_response;
                   case 3: for(i=0; i<nrhox; i++) a1[i]=FRACT[i][IXMG1]*STIM[i]/RHO[i];
                           return &OK_response;
                 }
               }
               else if(!strcmp(species->s, "Al1"))
               {
                 switch(key)
                 {
                   case 0: for(i=0; i<nrhox; i++) a1[i]=AL1OP_new(i)*FRACT[i][IXAL1]*STIM[i]/RHO[i];
                           return &OK_response;
                   case 1: for(i=0; i<nrhox; i++) a1[i]=AL1OP_new(i);
                           return &OK_response;
                   case 2: for(i=0; i<nrhox; i++) a1[i]=AL1OP(i);
                           return &OK_response;
                   case 3: for(i=0; i<nrhox; i++) a1[i]=FRACT[i][IXAL1]*STIM[i]/RHO[i];
                           return &OK_response;
                 }
               }
               else if(!strcmp(species->s, "Si1"))
               {
                 switch(key)
                 {
                   case 0: for(i=0; i<nrhox; i++) a1[i]=SI1OP_new(i)*FRACT[i][IXSI1]*STIM[i]/RHO[i];
                   return &OK_response;
                   case 1: for(i=0; i<nrhox; i++) a1[i]=SI1OP_new(i);
                   return &OK_response;
                   case 2: for(i=0; i<nrhox; i++) a1[i]=SI1OP(i);
                   return &OK_response;
                   case 3: for(i=0; i<nrhox; i++) a1[i]=FRACT[i][IXSI1]*STIM[i]/RHO[i];
                   return &OK_response;
                 }
               }
               else if(!strcmp(species->s, "Fe1"))
               {
                 switch(key)
                 {
                   case 0: for(i=0; i<nrhox; i++) a1[i]=FE1OP_new(i)*FRACT[i][IXFE1]*STIM[i]/RHO[i];
                   return &OK_response;
                   case 1: for(i=0; i<nrhox; i++) a1[i]=FE1OP_new(i);
                   return &OK_response;
                   case 2: for(i=0; i<nrhox; i++) a1[i]=FE1OP(i);
                   return &OK_response;
                   case 3: for(i=0; i<nrhox; i++) a1[i]=FRACT[i][IXFE1]*STIM[i]/RHO[i];
                   return &OK_response;
                 }
               }
               else if(!strcmp(species->s, "CH"))
               {
                 switch(key)
                 {
                   case 0: for(i=0; i<nrhox; i++) a1[i]=CHOP(i)*FRACT[i][IXCH]*STIM[i]/RHO[i];
                   return &OK_response;
                   case 1: for(i=0; i<nrhox; i++) a1[i]=CHOP(i);
                   return &OK_response;
                   case 2: for(i=0; i<nrhox; i++) a1[i]=CHOP(i);
                   return &OK_response;
                   case 3: for(i=0; i<nrhox; i++) a1[i]=FRACT[i][IXCH]*STIM[i]/RHO[i];
                   return &OK_response;
                 }
               }
               else if(!strcmp(species->s, "NH"))
               {
                 switch(key)
                 {
                   case 0: for(i=0; i<nrhox; i++) a1[i]=NHOP(i)*FRACT[i][IXNH]*STIM[i]/RHO[i];
                   return &OK_response;
                   case 1: for(i=0; i<nrhox; i++) a1[i]=NHOP(i);
                   return &OK_response;
                   case 2: for(i=0; i<nrhox; i++) a1[i]=NHOP(i);
                   return &OK_response;
                   case 3: for(i=0; i<nrhox; i++) a1[i]=FRACT[i][IXNH]*STIM[i]/RHO[i];
                   return &OK_response;
                 }
               }
               else if(!strcmp(species->s, "OH"))
               {
                 switch(key)
                 {
                   case 0: for(i=0; i<nrhox; i++) a1[i]=OHOP(i)*FRACT[i][IXOH]*STIM[i]/RHO[i];
                   return &OK_response;
                   case 1: for(i=0; i<nrhox; i++) a1[i]=OHOP(i);
                   return &OK_response;
                   case 2: for(i=0; i<nrhox; i++) a1[i]=OHOP(i);
                   return &OK_response;
                   case 3: for(i=0; i<nrhox; i++) a1[i]=FRACT[i][IXOH]*STIM[i]/RHO[i];
                   return &OK_response;
                 }
               }
               else
               {
                 snprintf(result, 511, "SME cannot compute continuous opacity for %s", species->s);
                 return result;
               }
             }
             else
             {
               for(i=0; i<nrhox; i++) a1[i]=ACOOL[i];  return &OK_response;
             }
    case  9: if(n>3)
             {
               species=(IDL_STRING *)arg[3];
               ...
               {
                 for(i=0; i<nrhox; i++) a1[i]=N1OP(i)*FRACT[i][IXN1 ]*STIM[i]/RHO[i];
                 return &OK_response;
               }
               else if(!strcmp(species->s, "O1"))
               {
                 for(i=0; i<nrhox; i++) a1[i]=O1OP(i)*FRACT[i][IXO1 ]*STIM[i]/RHO[i];
                 return &OK_response;
               }
               else if(!strcmp(species->s, "Mg2"))
               {
                 for(i=0; i<nrhox; i++) a1[i]=MG2OP(i)*FRACT[i][IXMG2]*STIM[i]/RHO[i];
                 return &OK_response;
               }
               else if(!strcmp(species->s, "Si2"))
               {
                 for(i=0; i<nrhox; i++) a1[i]=SI2OP(i)*FRACT[i][IXSI2]*STIM[i]/RHO[i];
                 return &OK_response;
               }
               else if(!strcmp(species->s, "Ca2"))
               {
                 for(i=0; i<nrhox; i++) a1[i]=CA2OP(i)*FRACT[i][IXCA2]*STIM[i]/RHO[i];
                 return &OK_response;
               }
               else
               {
             ...
             {
               for(i=0; i<nrhox; i++) a1[i]=ALUKE[i];  return &OK_response;
             }
    case 10: for(i=0; i<nrhox; i++) a1[i]=AHOT[i];   return &OK_response;
    case 11: for(i=0; i<nrhox; i++) a1[i]=SIGEL[i];  return &OK_response;
    case 12: for(i=0; i<nrhox; i++) a1[i]=SIGH2[i];  return &OK_response;
    default: strncpy(result, "Wrong opacity switch number", 511); return result;
  }
}
...
//}
    if(EXUP>=POTION[SPINDEX[LINE]])
    {
      if(!OPEN)
      {
        ...
        if(file12!=NULL) OPEN=1;
        if(OPEN) fprintf(file12,"Lines are numbered from 0\n");
      }
      AUTOION[LINE]=1;
      if(GAMQST[LINE]>0.0 && GAMVW[LINE]>0.0)
      {
        if(OPEN) fprintf(file12, "Autoionizing line \'%s\' #%d will be computed\n",
                 strtrim(Terminator(SPLIST+8*SPINDEX[LINE], 8)), LINE);
      }
      else
      {
        if(OPEN) fprintf(file12, "Autoionizing line \'%s\' #%d will not be computed\n",
                 strtrim(Terminator(SPLIST+8*SPINDEX[LINE], 8)), LINE);
        MARK[LINE]=2;
      }
    }
  }
  if(OPEN) fclose(file12);
...
//  species_list=(char *)calloc(NLINES, 8);
  ...
  {
    strncpy(result, "No enough space in EOS_count_species", 511); return result;
  }
...
//    return3                      =0;
  ...
  for(LINE=0; LINE<NLINES; LINE++)
  {
    strncpy(tmpname, spname+8*LINE, 8); tmpname[8]='\0';
    c=strchr(tmpname, ' ');
    if(c!=NULL) *c='\0'; /* Cut the ionization stage */
    strncpy(species_list+8*LINE, tmpname, 8);
    i=strlen(tmpname);
    if(i<8) for(; i<8; i++) species_list[8*LINE+i]=' ';
  }
...
/* First determine the size of the complete list returned by eqcount in as N_SPLIST */
  ...
  {
    case 0: break;
    case 1: FREE(species_list);
            strncpy(result, "EOS_count_species found illegal species name", 511);
            return result;
   default: FREE(species_list);
            snprintf(result, 511, "EOS_count_species - SPLSIZ must be larger than %d", i);
            return result;
  }
...
/* Now allocate space for the complete list of species and the index */
  ...
  if(SPLIST==NULL)
  {
    strncpy(result, "No enough space in EOS_count_species", 511);
    return result;
  }
...
//printf("%s\n", Terminator(species_list, 8)); 
  ...
  {
    case 0: break;
    case 1: FREE(species_list); FREE(SPLIST);
            strncpy(result, "EOS_list_species found illegal species name", 511);
            return result;
    case 2: FREE(species_list); FREE(SPLIST);
            strncpy(result, "EOS_list_species received too small N_SPLIST", 511);
            return result;
    case 3: FREE(species_list); FREE(SPLIST);
            strncpy(result, "EOS_list_species could not match ionization state", 511);
            return result;
    case 4: FREE(species_list); FREE(SPLIST);
            strncpy(result, "EOS_list_species found e- in the middle of the list", 511);
            return result;
   default: FREE(species_list); FREE(SPLIST);
            strncpy(result, "EOS_list_species - this error should never happen", 511);
            return result;
  }
  FREE(species_list);
  N_SPLIST=i;
...
//            i,NRHOX,i-1,FRACT[i-1],i-1,PARTITION_FUNCTIONS[i-1]);
    for(i=0; i<NRHOX_allocated; i++) FREE(FRACT[i]);
    FREE(FRACT);
  }
  if(PARTITION_FUNCTIONS!=NULL)
  {
    for(i=0; i<NRHOX_allocated; i++) FREE(PARTITION_FUNCTIONS[i]);
    FREE(PARTITION_FUNCTIONS);
  }
  flagIONIZ=0;

  if(POTION!=NULL) FREE(POTION);
  if(MOLWEIGHT!=NULL) FREE(MOLWEIGHT);
...
//    FRACT[i]=(float *)calloc(N_SPLIST, sizeof(float));
    ...
    if(FRACT[i]==NULL)
    {
      strncpy(result, "Ionization: Not enough memory", 511);
      return result;
    }
  }
...
/* Find out the location of continuous absorbers */
  ...
  {
    if(     !strncmp(SPLIST+8*i, "H ",    2)) IXH1   =i;
    else if(!strncmp(SPLIST+8*i, "H+ ",   3)) IXH2   =i;
    else if(!strncmp(SPLIST+8*i, "H- ",   3)) IXHMIN =i;
    else if(!strncmp(SPLIST+8*i, "H2 ",   3)) IXH2mol=i;
    else if(!strncmp(SPLIST+8*i, "H2+ ",  4)) IXH2pl =i;
    else if(!strncmp(SPLIST+8*i, "He ",   3)) IXHE1  =i;
    else if(!strncmp(SPLIST+8*i, "He+ ",  4)) IXHE2  =i;
    else if(!strncmp(SPLIST+8*i, "He++ ", 5)) IXHE3  =i;
    else if(!strncmp(SPLIST+8*i, "C ",    2)) IXC1   =i;
    else if(!strncmp(SPLIST+8*i, "Al ",   3)) IXAL1  =i;
    else if(!strncmp(SPLIST+8*i, "Si ",   3)) IXSI1  =i;
    else if(!strncmp(SPLIST+8*i, "Si+ ",  4)) IXSI2  =i;
    else if(!strncmp(SPLIST+8*i, "Ca ",   3)) IXCA1  =i;
    else if(!strncmp(SPLIST+8*i, "Ca+ ",  4)) IXCA2  =i;
    else if(!strncmp(SPLIST+8*i, "Mg ",   3)) IXMG1  =i;
    else if(!strncmp(SPLIST+8*i, "Mg+ ",  4)) IXMG2  =i;
    else if(!strncmp(SPLIST+8*i, "N ",    2)) IXN1   =i;
    else if(!strncmp(SPLIST+8*i, "Fe ",   3)) IXFE1  =i;
    else if(!strncmp(SPLIST+8*i, "O ",    2)) IXO1   =i;
    else if(!strncmp(SPLIST+8*i, "CH ",   3)) IXCH   =i;
    else if(!strncmp(SPLIST+8*i, "NH ",   3)) IXNH   =i;
    else if(!strncmp(SPLIST+8*i, "OH ",   3)) IXOH   =i;
    POTION[i]=-1.; MOLWEIGHT[i]=-1.;
  }
//for(j=0;j<NLINES;j++) printf("%d %d %s\n",j,SPINDEX[j],Terminator(SPLIST+8*(SPINDEX[j]-1),8));
  eos_mode=(use_electron_density_from_EOS)?0:10;
//  pf_mode=2;
//      strncpy(result, "Ionization: debug return0", 511);
//    return result;
  if(return_pfs)
  {
    for(i=0; i<NRHOX; i++)
...
//    {
//      strncpy(result, "Ionization: debug return1", 511);
//      return result;
//    }
//    if(i==NRHOX-18 && return2)
//    {
//      strncpy(result, "Ionization: debug return2", 511);
//      return result;
//    }
//    if(i==NRHOX-17 && return3)
//    {
//      strncpy(result, "Ionization: debug return3", 511);
//      return result;
//    }
...
if(dump01 && i==NRHOX-1)
{
  ...
  printf("N_SPLIST=%d, RHO[i]=%g\n", N_SPLIST, RHO[i]);
  for(j=0;j<N_SPLIST;j++) printf("%3d %8s %8f %14.4e %14.4e\n",j,Terminator(SPLIST+8*j,8),                                             PARTITION_FUNCTIONS[i][j],
                                             FRACT[i][j],
                                             FRACT[i][j]/RHO[i]);
...
//  printf("%d %g %g\n",i, POTION[SPINDEX[0]-1],POTION[SPINDEX[1]-1]);
    ...
    if(use_electron_density_from_EOS) XNE[i]=XNE_estim;
    if(use_particle_density_from_EOS) XNA[i]=XNA_estim;
...
//                FRACT[i][IXH2],Terminator(SPLIST+8*IXH2,8));
  ...
  {
    snprintf(result, 511,
           "WARNING: EOS-computed electron density differs from the model by %d%% in layer %d",
...
extern "C" char const * SME_DLL GetFraction(int n, void *arg[])
{
  ...
  if(!flagMODEL)
  {
    strncpy(result, "No model atmosphere has been set", 511);
    return result;
  }
  ...
  if(!strncmp("e-", a0->s, a0->slen)) mode=10; /* Ignore PF when dealing
                                                  with electrons */
  ...
  for(i=0; i<N_SPLIST; i++)    /* Search for requested species */
  {
    if(!strncmp(SPLIST+8*i, a0->s, a0->slen))
    {
      switch(mode)
      {
        case  0: for(j=0; j<min(NRHOX, l); j++) a[j]=              FRACT[j][i]*
                                                     PARTITION_FUNCTIONS[j][i];
                 return &OK_response;
        case  1: for(j=0; j<min(NRHOX, l); j++) a[j]=PARTITION_FUNCTIONS[j][i];
                 return &OK_response;
        default: for(j=0; j<min(NRHOX, l); j++) a[j]=              FRACT[j][i];
                 return &OK_response;
      }
    }
  }
  snprintf(result, 511, "Requested species %s not found", Terminator(a0->s, a0->slen));
  return result;
}
...
extern "C" char const * SME_DLL GetDensity(int n, void *arg[])
{
  ...
  for(j=0; j<min(NRHOX, l); j++) a[j]=RHO_eos[j];
  return &OK_response;
}
...
extern "C" char const * SME_DLL GetNatom(int n, void *arg[])
{
  ...
  l=*(short *)arg[0];          /* Array length */
  a=(double *)arg[1];          /* Array */
  for(j=0; j<min(NRHOX, l); j++) a[j]=XNA_eos[j];
  return &OK_response;
}
...
extern "C" char const * SME_DLL GetNelec(int n, void *arg[])
{
  ...
  l=*(short *)arg[0];          /* Array length */
  a=(double *)arg[1];          /* Array */
  for(j=0; j<min(NRHOX, l); j++) a[j]=XNE_eos[j];
  return &OK_response;
}
...
/* Get the arguments */
  ...
  if(n<9) {strncpy(result, "Not enough arguments", 511); return result;}
...
//printf("imu=%d, Impact=%g, height=%g, grazing=%d, %g\n", imu, P_impact/RADIUS, RAD_ATMO[0], grazing[imu], 1.-MU[imu]*MU[imu]);
      ...
      {
        for(nrhox=1;nrhox<NRHOX;nrhox++) if(P_impact>=RADIUS+RAD_ATMO[nrhox]) break;
...
extern "C" char const * SME_DLL GetLineRange(int n, void *arg[]) /* Get importance range for every line */
{
  ...
  if(n<2) // Check if arguments are present
  {
    strncpy(result, "GetLineRange: Requires an double array pointer and its length", 511);
    return result;
  }
...
/* Get the arguments */
  ...
  if(NMU > 81)
  {
    strncpy(result, "SME library is limited to maximum 81 mu angles", 511);
    return result;
  }
  MU     =(double *)arg[1]; /* Array of limb points */
  ...
  if(NWSIZE<NLINES)
  {
    strncpy(result, "Array size is smaller than the number of sp.lines", 511);
    return result;
  }
...
/* Wavelength grid is not preset. Construct an adaptive grid starting from the blue */
...
/* Now add one line point at each line center. Check line contribution. */
  ...
  for(line=0; line<NLINES; line++)
  {
    WW=WLCENT[line]; DWL_MIN=WW*DVEL_MIN/CLIGHTcm;
...
// Next pair of wavelength points associated with the next line
      ...
      if(Wlim_right[line]>WL[IWL] && WLCENT[line]<=WL[IWL] &&
        ALMAX[line]<EPS1) Wlim_right[line]=WL[IWL];
...
/*  AND NOW ADJUST STEP SIZE OF ABS(TABLE(IWL)-TABLE(IWL-1)) IS TOO BIG */
  ...
  while(IWL<NWL)
  {
    if(NWL>=NWSIZE-1) return 1;
    for(i=NWL; i>IWL; i--)
    {
      WL[i]=WL[i-1];
      for(IMU=0; IMU<NMU; IMU++) TABLE[i*NMU+IMU]=TABLE[(i-1)*NMU+IMU];
      if(long_continuum)
      {
        for(IMU=0; IMU<NMU; IMU++) FCBLUE[i*NMU+IMU]=FCBLUE[(i-1)*NMU+IMU];
      }
    }
    WL[IWL]=(WL[IWL]+WL[IWL-1])*0.5; NWL++;
...
/* ... and finally add one more point at the red end of the spectral interval */
  ...
  if(long_continuum)
  {
    for(IMU=0; IMU<NMU; IMU++) FCBLUE[IWL*NMU+IMU]=FCRED[IMU];
  }
  else
  {
    FNORM=(FCBLUE[0]+FCRED[0])*0.5;
...
/* Useful things for the Planck function */
  ...
  for(imu=0;imu<Nmu;imu++)
  {
    DBNU=2.0*(SRC_B-SRC_C)/((rhox[imu*NRHOX+NRHOX-1]-rhox[imu*NRHOX+NRHOX-2])*
             (OPC_B+OPC_C));
...
extern "C" char const * SME_DLL Contribution_functions(int n, void *arg[])
{
/*  This subroutine computes the contribution functions for all wavelengths
    and mu angles by returning the amount of intensity that reaches the surface
    from every atmospheric layer.

    Author: N.Piskunov

    LAST UPDATE: November 19, 2023.
    C++ Version: October 26, 2023
*/

  double *TABLE, *CTABLE, *WL, *MU, EPS1, EPS2;
  int NWSIZE, NWL;
  int imu, im;
  double MU_sph[MOSIZE], rhox[MUSIZE*MOSIZE], rhox_sph[MUSIZE][2*MOSIZE],
         P_impact, WW, delta_lambda;
...
/* Get the arguments */
  ...
  if(n<8)
  {
    strncpy(result, "Contribution_functions: Not enough arguments", 511); return result;
  }
  else
  {
    NMU=   *(short *)arg[0];  /* Number of limb points */
    MU=    (double *)arg[1];  /* Array of limb points */
    NWSIZE=  *(int *)arg[2];  /* Length of the arrays for synthesis */
    NWL=     *(int *)arg[3];  /* Length of predefined wavelength vector */
    WL=    (double *)arg[4];  /* Array for wavelengths */
    TABLE= (double *)arg[5];  /* Array for contribution functions */
    CTABLE=(double *)arg[6];  /* Continuum contribution functions, computed if long_continuum is set */
    long_continuum=*(short *)arg[7];
  }
...
//printf("im=%d, Rad=%g, P=%g, MU=%g\n", im, RAD_ATMO[im  ]+RADIUS, P_impact, MU[imu]);
    ...
    iret=Contrib_SPH(rhox_sph, NMU, NRHOXs, EPS1, EPS2, CTABLE, TABLE, NWSIZE, NWL,
                     WL, long_continuum, grazing);
  }
  else /* Plane-parallel case is handled by the Contrib_PP routine */
  {
    for(imu=0; imu<NMU; imu++) /* Prepare RHOX arrays for each Mu */
    {
      for(im=0; im<NRHOX; im++) rhox[imu*NRHOX+im]=RHOX[im]/MU[imu];
...
//  printf("0) NWL=%d, NWSIZE=%d, keep_lineop=%d\n",NWL,NWSIZE,keep_lineop);
    iret=Contrib_PP(rhox, NMU, EPS1, EPS2, CTABLE, TABLE, NWSIZE, NWL,
                    WL, long_continuum);
//  printf("1) NWL=%d, NWSIZE=%d, keep_lineop=%d\n",NWL,NWSIZE,keep_lineop);
  }
...
int Contrib_SPH(double rhox[][2*MOSIZE], int NMU, int NRHOXs[], double EPS1, double EPS2,
                double *CTABLE, double *TABLE, int NWSIZE, int &NWL,
                double *WL, short long_continuum, int grazing[])
{
/*  THIS SUBROUTINE CALLS SUBROUTINE Contrib_Inten_SPH TO INTEGRATE THE CONTRIBUTION
    FUNCTIONS OF SPECIFIC INTENSITIES (part of intensity that reaches the surface from every
    atmospheric layer) FOR CONTINUUM (returned as "CTABLE") AND FOR THE LINE+CONTINUUM
    (returned as "TABLE"). The layout of these arrays is: TABLE[Wavelength][Mu][Depth].

    Author: N.Piskunov

    UPDATES: 19-Nov-2023 written, based on the C++ version of the RINTS_SPH routine
*/
  double WW, FCL, FNORM;
  double opacity_tot[2*MOSIZE], opacity_cont[2*MOSIZE],
         source[2*MOSIZE], source_cont[2*MOSIZE];
  ...
  if(NWL>0 && NWL<=NWSIZE)
  {
    ...
    for(IWL=0; IWL<NWL; IWL++)
    {
      ...
      for(IMU=0; IMU<NMU; IMU++)
      {
        ...
        Contrib_Intens_SPH(nrhox, rhox[IMU], opacity_tot, source, TABLE+(IWL*NMU+IMU)*NRHOX, grazing[IMU]);
        if(long_continuum)
        {
          Contrib_Intens_SPH(nrhox, rhox[IMU], opacity_cont, source_cont, CTABLE+(IWL*NMU+IMU)*NRHOX, grazing[IMU]);
        }
      }
    }
  }
...
int Contrib_PP(double *rhox, int NMU, double EPS1, double EPS2,
               double *CTABLE, double *TABLE,
               int NWSIZE, int &NWL, double *WL,
               short long_continuum)
{
/*  THIS SUBROUTINE CALLS SUBROUTINE Contrib_Inten_PP TO INTEGRATE THE CONTRIBUTION
    FUNCTIONS OF SPECIFIC INTENSITIES (part of intensity that reaches the surface from every
    atmospheric layer) FOR CONTINUUM (returned as "CTABLE") AND FOR THE LINE+CONTINUUM
    (returned as "TABLE"). The layout of these arrays is: TABLE[Wavelength][Mu][Depth].
    ...
    UPDATES: 26-Oct-2023 written C++ version
*/
  double WW, FCL, FNORM;
  double opacity_tot[MOSIZE], opacity_cont[MOSIZE], source[MOSIZE],
         source_cont[MOSIZE];
  ...
  {                         // No adaptive grid in this case
    ...
    {
      OPMTRX(WL[IWL], opacity_tot, opacity_cont, source, source_cont, line_first, line_last);
      Contrib_Intens_PP(NMU, rhox, opacity_tot, source, TABLE+IWL*NMU*NRHOX);
      if(long_continuum)
      {
        Contrib_Intens_PP(NMU, rhox, opacity_cont, source_cont, CTABLE+IWL*NMU*NRHOX);
      }
    }
  }
  return 0;
...
void Contrib_Intens_PP(int Nmu, double rhox[], double opacity[], double source[],
            double RESULT[])
{
/*
  Contribution function calculator for the plane parallel geometry
  AUTHOR: N.Piskunov
  UPDATES:  October 26, 2023 Written based on TBINTG routine with quadratic DELO and Bezier splines
...
/* Useful things for the Planck function */
  ...
  for(imu=0;imu<Nmu;imu++)
  {
    ...
    INTENSITY[imu]=0.5*(SRC_B+SRC_C)+DBNU;  // Line intensity at the bottom
    SPRIME_SAVE[imu]=0.0;                   // Initialize S'
    RESULT[imu*NRHOX+NRHOX-1]=INTENSITY[imu]*FLUX_SCALE;
  }
...
*/
      B=ALPHA*SRC_B+BETA*SRC_A+GAMMA*CNTR_AB;
      for(JM=NRHOX-1; JM>IM; JM--) RESULT[imu*NRHOX+JM]*=EPS;
      RESULT[imu*NRHOX+IM]=B*FLUX_SCALE;

    }
  }
}

void Contrib_Intens_SPH(int NRHOX, double RHOX[], double opacity[], double source[],
                        double *RESULT, int grazing)
{
/*
  RT solver
...
/* Useful things for the Planck function */
  ...
  DBNU=2.0*(SRC_B-SRC_C)/((RHOX[NRHOX-1]-RHOX[NRHOX-2])*(OPC_B+OPC_C));
  INTENSITY =(grazing)?0.:0.5*(SRC_B+SRC_C)+DBNU;  // Line intensity at the boundary
  RESULT[NRHOX-1]=INTENSITY*FLUX_SCALE;
...
*/
    B=ALPHA*SRC_B+BETA*SRC_A+GAMMA*CNTR_AB;
    for(JM=NRHOX-1; JM>IM; JM--) RESULT[JM]*=EPS;
    RESULT[IM]=B*FLUX_SCALE;
  }
}
...
/* Identify Helium lines included in Dimitrijevic & Sahal-Brechot table;
   Stark damping for those will be computed in subroutine GAMHE */
      ...
      if(!strncmp(spname+8*LINE, "He ", 3) && !MARK[LINE])
      {
        ...
        {
          case  3819: IDHEL[LINE]= 0; break;
          case  3867: IDHEL[LINE]= 1; break;
          case  3871: IDHEL[LINE]= 2; break;
          case  3888: IDHEL[LINE]= 3; break;
          case  3926: IDHEL[LINE]= 4; break;
          case  3964: IDHEL[LINE]= 5; break;
          case  4009: IDHEL[LINE]= 6; break;
          case  4120:
          case  4121: IDHEL[LINE]= 7; break;
          case  4143: IDHEL[LINE]= 8; break;
          case  4168:
          case  4169: IDHEL[LINE]= 9; break;
          case  4437: IDHEL[LINE]=10; break;
          case  4471: IDHEL[LINE]=11; break;
          case  4713: IDHEL[LINE]=12; break;
          case  4921:
          case  4922: IDHEL[LINE]=13; break;
          case  5015:
          case  5016: IDHEL[LINE]=14; break;
          case  5047: IDHEL[LINE]=15; break;
          case  5875: IDHEL[LINE]=16; break;
          case  6678: IDHEL[LINE]=17; break;
          case  4026: IDHEL[LINE]=18; break;
          case  4387:
          case  4388: IDHEL[LINE]=19; break;
          default:    break;
        }
      }
...
/*  Qudratic Stark effect (if the constant is available, compute according
    ...
    tables are used. */
        ...
        {
          if(GAMQST[LINE]>0.0 || AUTOION[LINE]) GQST=GAMQST[LINE]*TEMP6;
          else
          {
            GQST=(ion-1)?5.42e-7*ENU4[LINE]*XNELEC/((ion+1)*(ion+1)):
                         2.26e-7*ENU4[LINE]*XNELEC;
          }
        }
...
//  Compute the Gamma function of X, this function is valid over the range 1<X<2
          ...
          GAMMAF=1.e0+(-0.5748646e0+(0.9512363e0+(-0.6998588e0+
                      (0.4245549e0-0.1010678e0*GX)*GX)*GX)*GX)*GX;
...
//  converting to the full width.
          ...
          GVW=GVW*(H1FRC+0.413*HE1FRC+
                  (flagH2broad?0.876*H2molFRC:0.))*1.e6*2.;
        }
...
//  if(LINE==272) printf("%4d %8g %15.6e %15.6e %15.6e %15.6e\n",ITAU,TEMPER,GAMRAD[LINE],GQST,GAMQST[LINE],TEMP6);
...
/*  Line absorption with the VOIGT function */
        ...
        if(ALINE1/OPCON>ALMAX[LINE]) ALMAX[LINE]=ALINE1/OPCON;
      }
      ALINE+=ALINE1;
//      if(abs(V)<1.e-4)
...
//if(wave==wlcent) printf("Computing cent H line: %d %d %d %d %g\n",ITAU,LINE,NBLO,NBUP,aline);
//        aline=hlinop_(wave,NBLO,NBUP,wlcent,temper,xnelec,
//                      h1frc,he1frc,dopl)*CLIGHTcm;
        ALINE=aline*LINEOP[ITAU][LINE]*wave*wave;
      }
      else   // Non-hydrogen line
      {
...
/* CUBIC INTERPOLATION OVER T=5000,10000,20000,40000 IN LOG SCALE */
    ...
    GAM=((TJ0*W[IND][J]+TJ1*W[IND][J-1]+TJ2*W[IND][J-2])*ANE
        +(TJ0*V[IND][J]+TJ1*V[IND][J-1]+TJ2*V[IND][J-2])*ANP)
        *1.884e3/(W[IND][4]*W[IND][4]);
    if(GAM<0.) GAM=0.;
